...
**QuantumShield Health** is a comprehensive, hybrid AI-Quantum public health platform designed to help authorities predict dengue outbreaks before they occur (predicting 1 to 3 months ahead), optimize healthcare resource deployment via Quantum Optimization algorithms, and automate decision-making processes through Large Language Models (LLM / RAG).
...
- Limited visibility into future disease hotspots (1 to 3 months into the future).
...
The forecasting engine predicts outbreak risks 1 to 3 months ahead using historical epidemiological data.
- **Code Implementation:** Built with Python (`xgboost` / XGBoost Regressor). The model is trained and validated on real HCDC (Ho Chi Minh City Center for Disease Control) data from 2001-2026.
...
### Layer 2: Quantum Resource Allocation Engine (QUBO)
...
- **Code Implementation:** Formulated as a Quadratic Unconstrained Binary Optimization (QUBO) problem using D-Wave's `dimod`. The engine calculates true ROI (Return on Investment) for medical teams and dynamically balances penalty strengths vs. resource constraints to allocate core resources (staff teams, beds, kits, IV fluids, fogging units).
- **Output:** The optimal allocation strategy is generated as a JSON payload (`allocation_output.json`) and immediately sent to the Dashboard for execution.

### Layer 3: RAG/LLM Copilot (GenAI Decision Protocol)
...
- **GenAI Decision Protocol (RAG):** The system integrates with LLMs (Google Gemini / OpenAI) through a Retrieval-Augmented Generation (RAG) pipeline to automatically draft highly professional medical dispatch orders based on official Dengue Prevention Guidelines (`docs/SXH.md`).
...
4. **Dengue Forecasting (AI):** Interactive charts displaying the XGBoost ML predictions validated on real HCDC data.
...
### Step 1: Start the System (Full Stack)
...
### Standalone AI Backend Testing (Python Only)
If you only want to test the ML, Quantum Allocation, and GenAI Dispatch Order generation locally without Docker:
```bash
# Install Python requirements
pip install -r requirements.txt

# 1. Run Quantum Allocation (Layer 2)
python 08_quantum_allocation.py

# 2. Run GenAI RAG Dispatch Order Generator (Layer 3)
